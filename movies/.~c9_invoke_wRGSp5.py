...
def index(request):
    ...
    '''
    user = request.user
    like_movies = user.like_movies.all().values('genres')
    genre_count = {}
    for genre in like_movies:
        if genre not in list(genre_count.keys()):
           genre_count[genre] = 1
        else:
            genre_count[genre] += 1
    top_genres = sorted(list(genre_count.values()), key=operator.itemgetter(1))[:10]
    recommendation = []
    i = 0
    while i < 10:
